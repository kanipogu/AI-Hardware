...
def simulate_threshold_hw(image_np):
    ...
    dynamic_thresh = int(np.mean(image_np))
    np.savetxt("verilog/threshold.txt", [dynamic_thresh], fmt="%d")
    
    with open("verilog/threshold.txt", "w") as f:
        f.write(str(dynamic_thresh))
